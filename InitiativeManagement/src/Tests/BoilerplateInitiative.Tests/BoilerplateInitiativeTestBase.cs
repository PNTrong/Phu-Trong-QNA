...
{
    public abstract class BoilerplateInitiativeTestBase : AbpIntegratedTestBase<BoilerplateInitiativeTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected BoilerplateInitiativeTestBase()
        {
        ...
        protected void UsingDbContext(Action<BoilerplateInitiativeDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BoilerplateInitiativeDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BoilerplateInitiativeDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BoilerplateInitiativeDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BoilerplateInitiativeDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerplateInitiativeDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BoilerplateInitiativeDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerplateInitiativeDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BoilerplateInitiativeDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BoilerplateInitiativeDbContext, Task<T>> func)
        {
            T result;
